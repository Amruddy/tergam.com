...
export function UserProfileModal({
  ...
  const body = (
    <FormCard title={forceSetup ? 'Личный кабинет' : 'Мой кабинет'} onClose={onClose ?? (() => {})}>
      <div className="space-y-4">
        <div className="flex items-center gap-3 rounded-2xl bg-slate-50 dark:bg-[#0a0a0f] border border-slate-200 dark:border-white/8 px-3.5 md:px-4 py-3">
          <div className="w-10 h-10 rounded-xl bg-indigo-500/15 text-indigo-500 flex items-center justify-center flex-shrink-0">
            <UserRound size={18} />
          </div>
        ...
        {!isAuthenticated && (
          <form onSubmit={handleAuth} className="space-y-3 rounded-2xl border border-slate-200 dark:border-white/8 p-3.5 md:p-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-2 pt-1">
            {!forceSetup && (
              <Btn type="button" variant="secondary" className="w-full sm:flex-1" onClick={onClose}>
                Закрыть
              </Btn>
            )}
            <Btn type="submit" variant="primary" className="w-full sm:flex-1" disabled={!fullName.trim()}>
              Сохранить
            </Btn>
          </div>
        </form>
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:px-4">
      <motion.div initial={{ opacity: 0, scale: 0.98, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="w-full max-w-lg rounded-t-[28px] md:rounded-2xl overflow-hidden">
        {body}
      </motion.div>
    </div>
  )
