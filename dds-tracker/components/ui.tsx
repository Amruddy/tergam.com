    ...
    <div className="flex flex-wrap items-start justify-between gap-4 min-w-0">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-[34rem] text-sm leading-snug text-slate-400 dark:text-gray-500">{subtitle}</p>
        </div>
      </div>
    ...
    <div className={`grid ${cols} gap-3`}>
      {items.map(({ label, value, icon: Icon, color }, i) => (
        <motion.div
          ...
          className="min-w-0 bg-white dark:bg-[#13131a] border border-slate-200/80 dark:border-white/[0.06] rounded-[22px] p-3.5 md:p-4 space-y-2.5 transition-colors duration-300"
        >
          <div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: `${color}18` }}>
            <Icon size={14} style={{ color }} />
          </div>
          <div className="min-w-0">
            <div className="text-[11px] md:text-xs text-slate-400 dark:text-gray-500 leading-snug">{label}</div>
            <div className="mt-1 text-[15px] md:text-base font-bold text-slate-900 dark:text-white leading-tight whitespace-nowrap overflow-hidden text-ellipsis">{value}</div>
          </div>
        </motion.div>
      ))}
    </div>
...
export function FormCard({
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-[#13131a] border border-slate-200/80 dark:border-white/[0.06] rounded-[28px] md:rounded-2xl overflow-hidden transition-colors duration-300 max-h-[min(88dvh,760px)] flex flex-col"
    >
      <div className="flex items-center justify-between px-4 md:px-5 py-3.5 border-b border-slate-100 dark:border-white/[0.05]">
        <h3 className="font-semibold text-slate-900 dark:text-white text-[15px]">{title}</h3>
        <button
          onClick={onClose}
          className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 dark:text-gray-500 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 transition-colors"
        >
          <X size={14} />
        </button>
      </div>
...
export function Btn({
  ...
  const base =
    'inline-flex min-h-11 items-center justify-center gap-1.5 font-semibold rounded-2xl transition-all focus:outline-none disabled:opacity-40 whitespace-nowrap flex-shrink-0 px-4'
...
export function IconBtn({
  ...
  return (
    <button
      className={cn('min-h-11 min-w-11 rounded-xl flex items-center justify-center transition-all focus:outline-none', variants[variant], className)}
      {...props}
    >
      {children}
    </button>
...
export function FieldLabel({ children }: { children: React.ReactNode }) {
  return <label className="mb-2 block px-0.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500 dark:text-gray-400">{children}</label>
}

export function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[11px] md:text-xs font-semibold text-slate-400 dark:text-gray-600 uppercase tracking-wider px-0.5">{children}</p>
}
    ...
    <div className="flex bg-slate-100 dark:bg-[#0d0d14] rounded-2xl p-1 gap-1">
      {(['expense', 'income'] as const).map((t) => (
        <button
          ...
          onClick={() => onChange(t)}
          className={cn(
            'flex-1 min-h-11 rounded-xl px-3 py-2.5 text-sm font-semibold transition-all duration-200',
            value === t
              ? t === 'expense'
...
export const inputCls =
  'w-full min-h-11 bg-slate-50 dark:bg-[#0a0a0f] border border-slate-200 dark:border-white/8 rounded-2xl px-4 py-2.5 text-[15px] text-slate-900 dark:text-white placeholder-slate-300 dark:placeholder-gray-600 focus:outline-none focus:border-indigo-400/70 dark:focus:border-indigo-500/50 transition-colors'
    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2.5">
      {cats.map((cat) => (
        <button
          ...
          className={cn(
            'flex min-h-[88px] flex-col items-center justify-center gap-2 rounded-[22px] border px-2 py-3.5 transition-all',
            value === cat.id
              ? 'border-2 shadow-sm'
        ...
        >
          <span className="text-[18px] sm:text-lg leading-none">{cat.emoji}</span>
          <span className={cn('text-[11px] text-center leading-tight font-medium', value === cat.id ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-gray-500')}>
            {cat.name}
          </span>
        </button>
      ))}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
      {accounts.map((account) => (
        <button
          ...
          className={cn(
            'flex min-h-[64px] items-center gap-3 rounded-[22px] border px-4 py-3.5 text-left transition-all',
            value === account.id
              ? 'border-2 shadow-sm'
              : 'border-slate-200 dark:border-white/8 hover:border-slate-300 dark:hover:border-white/15'
          )}
        ...
        >
          <span className="text-base leading-none">{account.emoji}</span>
          <span className="min-w-0">
            <span className={cn('block text-sm font-semibold truncate', value === account.id ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-gray-300')}>
              {account.name}
            </span>
            <span className="block text-[11px] text-slate-400 dark:text-gray-600 truncate">
